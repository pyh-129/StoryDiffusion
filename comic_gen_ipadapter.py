...
# from utils.ip_adapter.attention_processor import IPAttnProcessor2_0
...
from utils.ip_adapter import IPAdapterPlus
...
prompts = [
    ...
    # "a cartoon character is painting a mural on a large wall in a city square."
    # "standing.",
    # "standing at the edge of a vibrant, animated forest where trees hum cheerful tunes.",
    # "holding a glowing relic shaped like a quirky cartoon star, pulsating with mysterious energy.",
    # "walking through a candy-colored village, noticing strange symbols drawn on the walls of gingerbread houses.",
    # "finding a scroll with doodled instructions, addressed to a name suspiciously similar to their own.",
    # "gazing at the sky as a shooting star transforms mid-flight into a bouncing comet that lands nearby.",
    # "trapped inside a giant storybook where the pages come alive with moving illustrations.",
    # "chasing a mischievous shadow through a maze of swirling, animated alleys.",
    # "hearing a playful yet eerie melody calling from inside a glowing, oversized mushroom.",
    # "discovering a hidden compartment inside their cartoon backpack, containing a map to an unknown world.",
    # "standing on top of a wobbly hill, facing a storm of animated lightning bolts shaped like exclamation marks."
]
prompts = [
    "A boy in the forest.",
    "",
...
negative_prompt = "naked, deformed, bad anatomy, disfigured, poorly drawn face, mutation, extra limb, ugly, disgusting, poorly drawn hands, missing limb, floating limbs, disconnected limbs, blurry, watermarks, oversaturated, distorted hands, amputation,simple background"
...
# output_path = "./res/res_story/res_0.5"
# os.makedirs(output_path,exist_ok=True)
...
for real_prompt in real_prompts:
    ...
    print(real_prompt)
    real_images.append(ip_model.generate(pil_image=input_id_images[1],height = height,width = width,prompt = real_prompt, num_samples=1, num_inference_steps=50, scale = 0.5, seed=420,negative_prompt=negative_prompt)[0])
